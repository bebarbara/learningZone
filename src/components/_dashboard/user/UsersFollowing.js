...
import { UserListHead, UserListToolbar, UsersNotFound } from '.';
...
export default function UsersFollowing() {
  ...
  useEffect(() => {
    ...
    getUsersFollowing(handleAllUsers, currentUser.id, currentUser.token);
  }, []);
  const handleAllUsers = (response) => {
    console.log('set PostsFollowing', response);
    setAllUsers(response.following);
  };
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell align="center" colSpan={6} sx={{ py: 3 }}>
                        <UsersNotFound searchQuery={filterName} />
                      </TableCell>
                    </TableRow>
                  </TableBody>
                )}
